...
readSpots = function(loc, nreplicates = 1){
  ...
  aar1[nrow(aar1)+1, ] = c("ATCATGGACTACCGAC-1", "Red pulp") # manual fix 
  aar1 = aar1 %>%
    mutate(spot = Barcode,
...
prot = cbind(test$protein1, test$protein2)
RNA = cbind(test$RNA1, test$RNA2)

rna_size = unname(colSums(RNA)) / median(colSums(RNA))
protein_size = unname(colSums(prot)) / median(colSums(prot))

rna_1 = as.data.frame(t(test$RNA1)) %>%
  select(unname(sapply(fig_pairs, c))) %>%
  mutate(spot = colnames(test$RNA1),
         repl = 1,
         size_rna = rna_size[1:nrow(.)])

prot_1 = as.data.frame(t(test$protein1)) %>%
  mutate(spot = colnames(test$protein1),
         repl = 1,
         size_prot = protein_size[1:nrow(.)])

df_1 = full_join(rna_1, prot_1, by = "spot") %>%
  full_join(test$coords1, by = "spot") %>%
  full_join(test$AAR1, by = "spot")


rna_2 = as.data.frame(t(test$RNA2)) %>%
  select(unname(sapply(fig_pairs, c))) %>%
  mutate(spot = colnames(test$RNA2),
         repl = 2,
         size_rna = rna_size[(nrow(rna_1)+1):length(rna_size)])

prot_2 = as.data.frame(t(test$protein2)) %>%
  mutate(spot = colnames(test$protein2),
         repl = 2,
         size_prot = protein_size[(nrow(rna_1)+1):length(protein_size)])

df_2 = full_join(rna_2, prot_2, by = "spot") %>%
  full_join(test$coords2, by = "spot") %>%
  right_join(test$AAR2, by = "spot")
...
df$CD4 = c(df_1$CD4, df_2$CD4) # no idea
df$`F4-80`
test %>%
  ggplot() + geom_point(aes(x = imagecol, y = imagerow, color = `F4-80`))

test = SpotsProteinData("~/Documents/postdoc/MCAR/data/spots/spleen/", fig_pairs)
